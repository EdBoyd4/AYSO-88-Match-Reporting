...
function selectScheduledMatchOptionsFromDatabase(){
    global $dBConnection;
    // Start the transaction
    $dBConnection->begin_transaction();
    ...
    try {
        $sql = "SELECT 
            fields.field_number,
            fields.field_name,
            divisions_with_coordinators.division_number,
            divisions_with_coordinators.division_name,
            scheduled_matches.match_date,
            DATE_FORMAT(scheduled_matches.match_time, '%H:%i') AS formatted_match_time
        FROM scheduled_matches
        LEFT JOIN match_reports on scheduled_matches._id = match_reports.match_id
        LEFT JOIN divisions_with_coordinators 
            ON divisions_with_coordinators._id = scheduled_matches.division
        LEFT JOIN fields 
            ON fields._id = scheduled_matches.field
        WHERE  
            match_reports._id IS NULL
            AND (
                (scheduled_matches.match_date < ?)
                OR 
                (scheduled_matches.match_date = ? AND scheduled_matches.match_time < ?)
            )
            AND scheduled_matches.match_date >= NOW() - INTERVAL 5 DAY
        ORDER BY scheduled_matches.match_date ASC, scheduled_matches.division ASC, scheduled_matches.field ASC, scheduled_matches.match_time ASC;
        "; 
